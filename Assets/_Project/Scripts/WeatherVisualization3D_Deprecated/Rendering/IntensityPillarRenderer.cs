...
namespace WeatherVisualization3D
{
    ...
    public class IntensityPillarRenderer : MonoBehaviour, IVolumetricRenderer
    {
        ...
        private void CreateDefaultMaterial()
        {
            ...
            string path = "Assets/_Project/Materials/WeatherVisualization/IntensityPillarMaterial.mat";
            string dir = System.IO.Path.GetDirectoryName(path);
            if (!System.IO.Directory.Exists(dir))
            {
                System.IO.Directory.CreateDirectory(dir);
            }
            UnityEditor.AssetDatabase.CreateAsset(pillarMaterial, path);
            UnityEditor.AssetDatabase.SaveAssets();
            Debug.Log($"[IntensityPillarRenderer] Created pillar material asset at: {path}");
#endif
        }
