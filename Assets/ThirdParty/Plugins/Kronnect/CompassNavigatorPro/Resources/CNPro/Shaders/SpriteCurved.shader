Shader "CompassNavigatorPro/Sprite Curved"
{
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			#pragma multi_compile_local _ SCROLLABLE SCROLLABLE_180 SCROLLABLE_360
			...
			fixed4 frag(v2f IN) : SV_Target {
				...
				#if SCROLLABLE || SCROLLABLE_180 || SCROLLABLE_360
				{
					#if SCROLLABLE_180
						float uvx = (scrPos.xy + COMPASS_CENTER) * 90.0 + 135;
					#elif SCROLLABLE_360
						float uvx = (scrPos.xy + COMPASS_CENTER) * 180.0 + 90;
					#else
						float3 vpos = mul(_CompassIP, float4(scrPos.xy, 0.5, 1));
						float uvx = atan2(vpos.z, vpos.x) + 3.1415927 * 1.5;
						uvx *= 180.0 / 3.1415927; // optimized by compiler
					#endif
					uvx += _CompassAngle;
					IN.texcoord.x = uvx.x / 360.0;
				}
				#endif
