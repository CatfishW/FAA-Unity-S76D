...
namespace CompassNavigatorPro {
    ...
    public partial class CompassPro : MonoBehaviour {
                        ...
                        if (poi.compassIconDistanceText != null) {
                            ...
                            if (_showDistance && poi.iconShowDistance && poi.distanceToFollow > 0.1f) {
                                ...
                                if (poi.lastCompassIconDistance != poi.distanceToFollow) {
                                    ...
                                    poi.lastCompassIconDistanceText = distanceInNauticalMiles.ToString(format);
                                }
                    ...
                    if (lastDistance != nearestPOIDistance) {
                        ...
                        if (lastDistance >= titleMinPOIDistance) {
                            ...
                            if (_titleShowDistance) {
                                ...
                                // Convert meters to nautical miles (1 nm = 1852 meters)
                                float distanceInNauticalMiles = lastDistance / 1852f;
                                // Use dynamic precision: show more decimal places for smaller distances
                                string format = distanceInNauticalMiles < 1.0f ? "0.000nm" : 
                                               distanceInNauticalMiles < 10.0f ? "0.00nm" : 
                                               distanceInNauticalMiles < 100.0f ? "0.0nm" : "0nm";
                                titleText.Append(distanceInNauticalMiles.ToString(format));
                                titleText.Append(")");
        ...
        void UpdateHalfWindsAppearance() {
            ...
            // Set longer tick for NSWE (cardinals)
            float cardinalTickHeight = _halfWindsHeight * 2.0f; // or any multiplier you prefer
            float ordinalTickHeight = _halfWindsHeight;
            // Vector4: (width, cardinalHeight, ordinalHeight, interval)
            compassBarMat.SetVector(ShaderParams.TicksSize, new Vector4(_halfWindsWidth, cardinalTickHeight, ordinalTickHeight, _halfWindsInterval));
        }
